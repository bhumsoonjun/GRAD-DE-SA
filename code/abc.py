...
def schwefel_2_3(x: np.ndarray):
    cost = 418.9829 * len(x) - np.sum(np.apply_along_axis(lambda y: y * np.sin(np.sqrt(np.abs(y))), 0, x))
    return cost
...
def ackley(x: np.ndarray):
    # ackley function
    cost = -20 * np.exp(-0.2 * np.sqrt(1/len(x) * np.sum(np.apply_along_axis(lambda y: y**2, 0, x)))) - np.exp(1/len(x) * np.sum(np.apply_along_axis(lambda y: np.cos(2 * np.pi * y), 0, x))) + 20 + np.exp(1)
    return cost

def rastrigin(x: np.ndarray):
    return 10 * len(x) + np.sum(np.apply_along_axis(lambda y: y**2 - 10 * np.cos(2 * np.pi * y), 0, x))

def dejong_5(x: np.ndarray):
    return bf.DeJong5()(x)

def dejong_3(x: np.ndarray):
    return bf.DeJong3(n_dimensions=len(x))(x) + 120
...
for test_name, test_func, test_bounds, vtr in test_funcs:
    for i in range(1):
        ...
        model.fit()
        print(metric.best_val, model.best_food_source.position, test_func(model.best_food_source.position), model.best_food_source.fit)
